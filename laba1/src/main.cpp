...
int main() {
  ...
  Rectangle rectangel({200, 200}, 100, 50);
  ...
  while (window.isOpen()) {
    ...
    switch (event.type) {
      ...
      case sf::Event::KeyPressed:
        if (isTranslationKey(event.key.code))
          translate(rectangel, event.key.code);
        break;

      case sf::Event::MouseWheelScrolled:
        scale(rectangel, event.mouseWheelScroll.delta);
        break;

      case sf::Event::MouseButtonPressed:
        rotate(rectangel, event.mouseButton.button);
        break;
    ...
    window.clear();
    window.draw(rectangel.getShape());
    window.display();
  }
